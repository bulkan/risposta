...
describe('async', function(){
  ...
  describe('implemented using bluebird', function(){
    ...
    it(' .compose binding',  function(done) {
      var testcontext = {name: 'foo'};

      var add2 = function(n, cb) {
        this.should.be.eql(testcontext);
        setTimeout(function() {
          cb(null, n + 2);
        }, 3);
      };
      ...
      var add2mul3 = async.compose(mul3, add2).bind(testcontext);
      add2mul3(3).bind(testcontext).then(function(result) {
        this.should.be.eql(testcontext);
